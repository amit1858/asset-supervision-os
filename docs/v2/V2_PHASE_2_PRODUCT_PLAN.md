...
> - **Source mode ≠ freshness (five orthogonal dimensions).** `sourceMode` is typed as the existing **`SourceMode`** (`local`\|`snowflake` — how/where sourced), **not** `IntegrationState`; integration health is carried separately and optionally as `integrationState?: IntegrationState` (only where a concrete consumer needs it). `freshness` (`fresh|stale|missing|unknown`, never `synthetic`) is temporal relative to `asOf`. `provenance` and derived `trustClassification` are the epistemic/presentation axes. Windows: sensor 15 min; production/OEE, CMMS, inventory, turnaround, financial 24 h; evaluated against the canonical **`ANCHOR_NOW = 2026-07-27`** clock, passed explicitly (never `Date.now()`). (Tech plan §10.2.)
