...
> 3. **Source mode and freshness are ORTHOGONAL** (owner Decision 4, 2026-07-31, corrected). Do **not** model `fresh|stale|missing|synthetic` as one axis, and do **not** type `sourceMode` as `IntegrationState`. `sourceMode` uses the existing **`SourceMode`** (`local`\|`snowflake`); integration health is assessed **separately** and is **not** a `ValueEnvelope` field (owner reconciliation — Slice 2.1a as built — 2026-08-02); `freshness` is value-temporal only — `fresh|stale|missing|unknown` — and **never** includes `synthetic`. A synthetic observation may be fresh or stale; synthetic disclosure comes from that separate integration assessment, never via `freshness`/`sourceMode`. Windows are selected by `freshnessClass`: condition signals 15 min; production/OEE, CMMS, inventory, turnaround, financial 24 h. This supersedes the "24h" and any `synthetic`-as-freshness proposal below. See technical plan §10.2.
...
> **⚠️ Owner reconciliation — Slice 2.1a as built — 2026-08-02.** Slice 2.1a is implemented and committed (`5960e06`). Three envelope contracts differ from earlier drafts and the **implementation is authoritative**:
> 1. **`ValueEnvelope` carries no `integrationState`.** Integration health remains a separate source/integration assessment over the existing `src/domain/integration.ts` model. Of the five orthogonal axes, the envelope carries `sourceMode`, `freshness`, `provenance` and the derived `trustClassification`; `integrationState` is **not** an envelope field.
> 2. **Supersession is forward-only.** A new version records `supersedesId` naming the version it replaces; `supersededByEventId` does not exist and a prior envelope is **never** backward-stamped, re-written or re-emitted as a modified copy. There is no `markSuperseded` helper.
> 3. **The freshness policy selector is `freshnessClass`**, a closed union (`condition_signal`, `production_oee`, `cmms_work_order`, `inventory_material`, `turnaround_readiness`, `financial_value`). An explicit class wins; otherwise a conservative `SourceKey` default applies; an ungoverned source resolves to `unknown` and is never guessed. `freshness.v1` remains the policy version.
>
> Also as built: `trustClassification` is derived internally from `provenance` and can never be supplied by a caller; an `unavailable` envelope has `value === null` **and** a non-empty `unavailableReason`, while numeric `0` remains a legitimate available value. See technical plan §11 and §15.3.
...
All objects share a common envelope: a stable `id`, a monotonic `version`, a `status`, a `trustClassification` wherever the object holds a derived value, a `provenance` reference, a `freshness` timestamp, and a `createdByEventId` link plus a **forward-only `supersedesId`** naming the version this one replaces. A prior version is never backward-stamped or modified; whether it has been superseded is derived from the successor relationship in the append-only ledger (owner reconciliation — Slice 2.1a as built — 2026-08-02). Objects are asset-agnostic; K-201 is one `Asset` instance.
