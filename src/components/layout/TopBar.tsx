...
/**
 * Top bar (tier 1): strong enterprise-blue (light) / deep-navy (dark) chrome.
 * Grouped into three zones — identity, global search, and account/context
 * controls. Search collapses to an icon at constrained widths (HeaderSearch).
 */
export function TopBar({
  ...
  return (
    <div data-app-header className="relative flex h-[54px] items-center gap-3 bg-header-bg px-4 text-header-fg">
      ...
      {/* Global search (inline at wide widths, icon-triggered when constrained) */}
      <HeaderSearch />

      <div className="ml-auto flex items-center gap-1">
        <VoiceHeaderButton />
        <HeaderIconButton label={`Notifications (${notifications})`} icon="bell" count={notifications} />
        <HeaderIconButton label={`My Approvals (${approvals})`} icon="approvals" count={approvals} href="/reliability" />
